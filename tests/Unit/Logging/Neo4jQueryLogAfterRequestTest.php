...
class Neo4jQueryLogAfterRequestTest extends TestCase
{
    ...
    public function test_skips_laravel_write_in_production_with_custom_channel_unless_allowed(): void
    {
        $path = tempnam(sys_get_temp_dir(), 'neo4j_laravel_prod_');
        $this->assertNotFalse($path);

        $this->app['env'] = 'production';
        $this->app['config']->set('neo4j-laravel.query_log_channel', 'neo4j_queries');
        $this->app['config']->set('neo4j-laravel.query_log_allow_production', false);
        $this->app['config']->set('logging.default', 'test_default');
        $this->app['config']->set('logging.channels.test_default', [
            'driver' => 'single',
            'path' => $path,
        ...
        $connection = $this->app->make('db')->connection('neo4j');
        $this->assertInstanceOf(Neo4jConnection::class, $connection);
        $connection->logQuery('MATCH (x) RETURN x', [], 2.0);

        Neo4jQueryLogAfterRequest::flush($this->app);

        $this->assertSame('', trim((string) file_get_contents($path)));
        $this->assertSame([], $connection->getQueryLog());

        @unlink($path);
    }

    public function test_writes_in_production_when_allow_flag_true(): void
    {
        $path = tempnam(sys_get_temp_dir(), 'neo4j_laravel_prod_allow_');
        $pathDefault = tempnam(sys_get_temp_dir(), 'neo4j_laravel_unused_');
        $this->assertNotFalse($path);
        $this->assertNotFalse($pathDefault);

        $this->app['env'] = 'production';
        $this->app['config']->set('neo4j-laravel.query_log_channel', 'neo4j_queries');
        $this->app['config']->set('neo4j-laravel.query_log_allow_production', true);
        $this->app['config']->set('logging.default', 'test_default');
        $this->app['config']->set('logging.channels.test_default', [
            'driver' => 'single',
            'path' => $pathDefault,
        ...
        $connection = $this->app->make('db')->connection('neo4j');
        $connection->logQuery('RETURN 2', [], 0.5);

        Neo4jQueryLogAfterRequest::flush($this->app);

        $this->assertStringContainsString('RETURN 2', (string) file_get_contents($path));
        $this->assertSame([], $connection->getQueryLog());
